...
def generate_test_experiments(n=2):
  ...
  experiments.append(Experiment(crystal=crystal, scan=scan,
...
test_input = [[{'n_removed': 0}, 'no_more_removed'],
  [{'n_removed': 100, 'cumul_percent_removed' : 50}, 'max_percent_removed'],
  [{'n_removed': 10, 'cumul_percent_removed' : 1,
    'merging_stats' : {'completeness' : 90}}, 'below_completeness_limit'],
  [{'n_removed': 10, 'cumul_percent_removed' : 1,
    'merging_stats' : {'completeness' : 98}}, 'max_cycles']
]
...
def test_scale_and_filter_termination(last_cycle_results, expected_termination):
  ...
  scale_and_filter = ScaleAndFilter(params, params.scale, params.delta_cc_half,
    _scaling_script, _scaling_script)
  with mock.patch.object(scale_and_filter, 'log_cycle_results', return_value=mock_res):
...
def test_scale_and_filter_results_logging():
  ...
  with mock.patch.object(results, '_parse_merging_stats', side_effect=_parse_side_effect):
...
def test_compute_delta_cchalf_returned_results():
  ...
  sorted_data, sorted_ccs = DeltaCCHalfScript.sort_deltacchalf_values(delta_cc, summary)
  ...
  assert summary['per_dataset_delta_cc_half_values']['delta_cc_half_values'] == \
    expected_cc_order
  ...
  _ = DeltaCCHalfScript.remove_datasets_below_cutoff(exp, refls, ids_to_remove,
    results_summary)
  ...
  refls = generate_test_reflections(2)
  ids_to_remove = [0, 1]
  image_group_to_expid_and_range = {0 : (0, (1, 5)), 1: (0, (6, 10)),
    2: (1, (1, 5)), 3: (1, (6, 10))}
  results_summary = {'dataset_removal': {}}
  _ = DeltaCCHalfScript.remove_image_ranges_below_cutoff(exp, refls, ids_to_remove,
    image_group_to_expid_and_range, results_summary)
  assert 'experiments_fully_removed' in results_summary['dataset_removal']
  assert 'n_reflections_removed' in results_summary['dataset_removal']

...
from dials.algorithms.scaling.plots import plot_scaling_models, statistics_tables,\
  plot_outliers, normal_probability_plot
...
def register_default_scaling_observers(script):
  ...
  script.register_observer(
    event='run_script', observer=ScalingHTMLGenerator(), callback='make_scaling_html')
...
class ScalingHTMLGenerator(Observer):
  ...
  def make_scaling_html(self, scaling_script):
    ...
    html = template.render(page_title='DIALS scaling report',
...
class ScalingOutlierObserver(Observer):
  ...
  def update(self, scaler):
    ...
    for scaler in active_scalers:
      ...
      outlier_isel = scaler.suitable_refl_for_scaling_sel.iselection().select(
        scaler.outliers)
      x, y, z = scaler.reflection_table['xyzobs.px.value'].select(
        outlier_isel).parts()
      self.data[id_] = {'x': list(x), 'y': list(y), 'z' : list(z),
        'image_size' : scaler.experiment.detector[0].get_image_size(),
        'z_range' : [i/ scaler.experiment.scan.get_oscillation()[1] for i in
          scaler.experiment.scan.get_oscillation_range()]}
  ...
  def make_plots(self):
    ...
    for key in sorted(self.data.keys()):
      ...
      d.update({'outlier_plot_'+str(key) : outlier_plots['outlier_xy_positions']})
...
class ErrorModelObserver(Observer):
  ...
  def update(self, scaler):
    self.data['delta_hl'] = list(scaler.experiment.scaling_model.error_model.delta_hl)

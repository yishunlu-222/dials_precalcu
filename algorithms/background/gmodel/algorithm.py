...
class BackgroundAlgorithm(object):
  ...
  def __init__(self, experiments, params=None):
    '''
    ...
    self._creator = Creator(experiments[0], params)

  def next_image(self, frame, image, mask, reflections):
    if not self._creator.initialized():
      self._creator.initialize()
    self._creator.next_image(frame, image, mask, reflections)
  ...
  def compute_background(self, reflections):
    ...
    '''
    from dials.array_family import flex
    if not self._creator.finalized():
      self._creator.finalize()

    # Do the background subtraction
    return self._creator.compute(reflections)
